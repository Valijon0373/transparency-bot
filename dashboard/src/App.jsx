...
export default function App() {
  ...
  const handleExportExcel = () => {
    ...
    const exportData = appeals.map((item, index) => ({
      ...
      "Murojaatchi Ismi": item.is_anonymous ? "Anonim" : (item.first_name || "Noma'lum"),
      "Telegram Username": item.username ? `@${item.username}` : (item.is_anonymous ? 'Anonim' : '-'),
      "Telefon Raqam": item.phone_number || (item.is_anonymous ? 'Anonim' : '-'),
